...
#   - --disable-sound-format-error: the sfx are .wav, which pygbag rejects by
#     default but browsers play fine (verified 2026-09-23).
#   - build/ is removed first because a stale blueball.apk is served in
...
PYTHONPATH= "$PYGBAG" --build --disable-sound-format-error .
...
# rather than assuming blueball.apk.
...
if [ "${1:-}" = "--serve" ]; then
    ...
    echo "reached main.py; the dark-blue menu means the game is running."
    PYTHONPATH= "$PYGBAG" --disable-sound-format-error .
fi
